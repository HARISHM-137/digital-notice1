-- ============================================================
-- CO-PO ATTAINMENT SYSTEM — FINAL COMPLETE SCHEMA
-- Single file solution: Cleanup + Create + Migrate
...
-- ============================================================
-- SECTION 0: CLEANUP (Drop conflicting tables)
-- ============================================================

-- Drop assessment tables that might have schema conflicts
...
-- 4. Subjects (FIXED: All columns present)
...
-- Create policies (idempotent with DO block)
DO $$ 
BEGIN
    -- Departments
    ...
    END IF;
    IF NOT EXISTS (SELECT 1 FROM pg_policies WHERE tablename = 'subjects' AND policyname = 'Allow all for authenticated') THEN
        CREATE POLICY "Allow all for authenticated" ON subjects FOR ALL TO authenticated USING (true);
    END IF;
    ...
    END IF;
    IF NOT EXISTS (SELECT 1 FROM pg_policies WHERE tablename = 'course_outcomes' AND policyname = 'Allow all for authenticated') THEN
        CREATE POLICY "Allow all for authenticated" ON course_outcomes FOR ALL TO authenticated USING (true);
    END IF;
    ...
    END IF;
    IF NOT EXISTS (SELECT 1 FROM pg_policies WHERE tablename = 'program_outcomes' AND policyname = 'Allow all for authenticated') THEN
        CREATE POLICY "Allow all for authenticated" ON program_outcomes FOR ALL TO authenticated USING (true);
    END IF;
    ...
    END IF;
    IF NOT EXISTS (SELECT 1 FROM pg_policies WHERE tablename = 'co_po_mapping' AND policyname = 'Allow all for authenticated') THEN
        CREATE POLICY "Allow all for authenticated" ON co_po_mapping FOR ALL TO authenticated USING (true);
    END IF;
    
    -- Assessment Tables (permissive for authenticated)
...
BEGIN
    ...
    RAISE NOTICE '📊 Created 25 tables with all assessment modules';
