    ...
    const fetchSubjects = async () => {
        try {
            ...
            if (!user) {
                ...
                const formatted = (data || []).map((s: any) => ({ label: `${s.code} - ${s.name}`, value: s.id }));
            ...
            if (formatted.length === 0) {
                const { data: allSubs } = await supabase.from("subjects").select("id, name, code").order("name");
                formatted = (allSubs || []).map((s: any) => ({ label: `${s.code} - ${s.name}`, value: s.id }));
            }
