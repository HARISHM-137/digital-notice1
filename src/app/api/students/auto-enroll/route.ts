...
export async function POST(request: NextRequest) {
    try {
        ...
        // Limit to exactly 6 subjects (even-semester policy)
        const subjectsToEnroll = subjects.slice(0, 6);
        const warning = subjects.length < 6
            ? `Only ${subjects.length} subjects available for semester ${semesters[0]} (expected 6)`
            : null;
