...
export default function AdminDashboard() {
    ...
    const [surveyStats, setSurveyStats] = useState<SurveyStats>({ totalSurveys: 0, activeSurveys: 0, totalResponses: 0, totalStudents: 0, completionPercentage: 0 });
    ...
    useEffect(() => {
        const fetchDashboard = async () => {
            try {
                ...
                // Fetch survey completion stats
                console.log("[Dashboard] Fetching survey stats...");
                try {
                    ...
                    const uniqueRespondents = new Set((responses || []).map((r: any) => r.respondent_id)).size;
                    const totalStudentCount = studs.count || 0;

                    const completionPct = totalStudentCount > 0
                        ? Math.round((uniqueRespondents / totalStudentCount) * 100)
                        : 0;
                    ...
                    console.log("[Dashboard] Survey stats:", { uniqueRespondents, totalStudentCount, completionPct });
                } catch (surveyError) {
                    console.warn("[Dashboard] Survey stats failed (table may not exist):", surveyError);
                }
            } catch (err) {
        ...
        <div className="space-y-6">
            ...
            <Card>
                <h3 className="text-lg font-bold text-slate-800 mb-4 flex items-center gap-2">
                    <span className="text-2xl">📊</span> Survey Completion
                </h3>
                <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-4">
                    <div className="bg-gradient-to-br from-blue-50 to-indigo-50 rounded-xl p-4 border border-blue-100">
                        <p className="text-sm text-blue-600 font-medium">Total Surveys</p>
                        <p className="text-2xl font-bold text-blue-800">{surveyStats.totalSurveys}</p>
                        <p className="text-xs text-blue-500">{surveyStats.activeSurveys} active</p>
                    </div>
                    <div className="bg-gradient-to-br from-emerald-50 to-teal-50 rounded-xl p-4 border border-emerald-100">
                        <p className="text-sm text-emerald-600 font-medium">Students Responded</p>
                        <p className="text-2xl font-bold text-emerald-800">
                            {surveyStats.totalResponses} / {surveyStats.totalStudents}
                        </p>
                        <p className="text-xs text-emerald-500">students</p>
                    </div>
                    <div className="bg-gradient-to-br from-purple-50 to-fuchsia-50 rounded-xl p-4 border border-purple-100">
                        ...
                        <p className="text-2xl font-bold text-purple-800">{surveyStats.completionPercentage}%</p>
                        <p className="text-xs text-purple-500">of enrolled students</p>
                    </div>
                </div>
                {/* Progress Bar */}
                <div className="space-y-2">
                    <div className="flex items-center justify-between text-sm">
                        ...
                        <span className="text-slate-800 font-bold">{surveyStats.totalResponses} / {surveyStats.totalStudents} students</span>
                    </div>
                    <div className="w-full bg-slate-100 rounded-full h-4 overflow-hidden">
                        <div
                            className="h-full rounded-full bg-gradient-to-r from-indigo-500 to-purple-500 transition-all duration-700"
                            style={{ width: `${Math.min(surveyStats.completionPercentage, 100)}%` }}
                        />
                    </div>
                </div>
            </Card>
